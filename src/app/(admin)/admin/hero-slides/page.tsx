...
export default function HeroSlidesPage() {
  ...
  async function load() {
    setLoading(true);
    // Load all slides including inactive for admin
  ...
  async function handleAdd() {
    ...
    const res = await fetch("/api/hero-slides", {
      ...
      body: JSON.stringify({ image_url: previewUrl, caption, sort_order: slides.length }),
    });
    if (res.ok) {
    ...
    <div>
      ...
      <div className="bg-tunnel-800 border border-tunnel-600 rounded-xl p-6 mb-8">
        ...
        <div className="mb-4">
          ...
          {previewUrl ? (
            ...
            <label className="flex flex-col items-center justify-center h-40 border-2 border-dashed border-tunnel-600 rounded-lg cursor-pointer hover:border-signal-amber/50 transition-colors">
              {uploading ? (
                <Loader2 className="w-8 h-8 animate-spin text-signal-amber mb-2" />
              ) : (
        ...
        <div className="mb-5">
          ...
          <input
            ...
            placeholder="e.g. Tunnel ELV Control Room — Project 2024"
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {slides.map((slide, i) => (
            <div key={slide.id} className={`bg-tunnel-800 border rounded-xl overflow-hidden flex items-center gap-4 p-3 ${slide.active ? "border-tunnel-600" : "border-tunnel-700 opacity-60"}`}>
              {/* Thumbnail */}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 mt-0.5">Slide {i + 1} • {slide.active ? "Visible" : "Hidden"}</p>
              </div>
              {/* Controls */}
              <div className="flex items-center gap-1 shrink-0">
                ...
                <button onClick={() => toggleActive(slide)} className="p-2 rounded-lg text-gray-400 hover:text-signal-amber hover:bg-tunnel-700 transition-colors" title={slide.active ? "Hide" : "Show"}>
